...
function Board({ rows, cols, mines }: BoardProps) {
  ...
  function revealEmpty(
    ...
    if (!first && tileState.surroundingMines && tileState.surroundingMines > 0)
      return;

    const adjacentTiles = getSurroundingTiles(tileState, newBoardState);

    for (let tile of adjacentTiles) {
      if (tile && !tile.revealed && !tile.hasMine)
        revealEmpty(tile, newBoardState, false);
    }
  }
  ...
  function getSurroundingTiles(
    ...
    if (tileState.row > 1)
      surroundingTiles.push(
        newBoardState[calculateIndex(tileState.row - 1, tileState.col, cols)]
      );
    ...
    if (tileState.col > 1)
      surroundingTiles.push(
        newBoardState[calculateIndex(tileState.row, tileState.col - 1, cols)]
      );
    ...
    if (tileState.row > 1 && tileState.col > 1)
      surroundingTiles.push(
        newBoardState[
          calculateIndex(tileState.row - 1, tileState.col - 1, cols)
        ]
      );
